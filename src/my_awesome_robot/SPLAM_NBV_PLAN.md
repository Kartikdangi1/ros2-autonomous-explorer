...
Launches: NBV SPLAM node + frontend_node (obstacle detection). Does NOT launch separate SLAM nodes (SPLAM node handles its own localization + mapping).
...
1. `colcon build --packages-select my_awesome_robot --symlink-install`
2. Launch: `ros2 launch my_awesome_robot nbv_splam.launch.py`
