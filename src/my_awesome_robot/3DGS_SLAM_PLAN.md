...
| File | Purpose |
|------|---------|
| `src/my_awesome_robot/my_awesome_robot/gsplat_slam.py` | Core library: `GaussianMap`, `GaussianInitializer`, `GaussianOptimizer`, `GaussianRenderer`, helpers |
| `src/my_awesome_robot/scripts/gsplat_slam_node.py` | ROS2 node executable |
| `src/my_awesome_robot/config/gsplat_params.yaml` | All parameters |
| `src/my_awesome_robot/launch/gsplat_slam.launch.py` | Standalone launch (includes Gazebo + EKF + SLAM Toolbox + 3DGS) |

## Files to Modify

| File | Change |
|------|--------|
| `src/my_awesome_robot/CMakeLists.txt` | Add `scripts/gsplat_slam_node.py` to `install(PROGRAMS)` |
| `src/my_awesome_robot/package.xml` | Add `<exec_depend>python3-torch</exec_depend>` comment block for gsplat pip dep |
...
6. **Test:** `colcon build` → `ros2 launch my_awesome_robot gsplat_slam.launch.py` → verify node starts, logs "Keyframe N accepted"
...
```bash
# Build
colcon build --packages-select my_awesome_robot --symlink-install

# Import check
python3 -c "from my_awesome_robot.gsplat_slam import GaussianMap, rgbd_to_pointcloud; print('OK')"

# Node starts
ros2 launch my_awesome_robot gsplat_slam.launch.py
