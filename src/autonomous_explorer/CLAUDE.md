...
Build both packages together since `autonomous_explorer` depends on `sensor_fusion`.
...
| Node | File | Role |
|------|------|------|
| `sensor_fusion_node` | `sensor_fusion/` package | Merges LiDAR + radar + RGB-D depth into `/fused_scan` using per-bin priority (LiDAR > Depth > Radar) |
| `obstacle_cluster_node` | `scripts/obstacle_cluster_node.py` | Clusters `/scan` into `/detected_obstacles` and `/obstacle_markers` |
...
### NBV Planning Library (`autonomous_explorer/nbv_utils.py`)

Core algorithms live in the Python package (not `scripts/`), importable as `from autonomous_explorer.nbv_utils import ...`:
...
| `/fused_scan` | LaserScan | sensor_fusion |
| `/imu` | Imu | Gazebo bridge |
| `/odom` | Odometry | Gazebo bridge (wheel odometry) |
| `/map` | OccupancyGrid | SLAM Toolbox |
| `/detected_obstacles` | PoseArray | obstacle_cluster_node |
| `/detected_obstacles_fusion` | PoseArray | sensor_fusion |
...
- `nbv_utils.py` is a library module (no `main()`); node scripts import from it via `from autonomous_explorer.nbv_utils import ...`
